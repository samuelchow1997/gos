...
function Leona:LoadMenu()
    ...
    GamCore:OnEnemyHeroLoad(function(hero) self.tyMenu.Combo.useon:MenuElement({id = hero.charName, name = hero.charName, value = true}) end)
    ...
    GamCore:OnEnemyHeroLoad(function(hero) self.tyMenu.Harass.useon:MenuElement({id = hero.charName, name = hero.charName, value = true}) end)
...
function Leona:Combo()
    ...
    if IsValid(target) then
        ...
        if self.tyMenu.Combo.UseE:Value() and Ready(_E) and myHero.pos:DistanceTo(target.pos) <= 875 then
        ...
        if self.tyMenu.Combo.UseW:Value() and Ready(_W) and myHero.pos:DistanceTo(target.pos) <= 325 then
            Control.CastSpell(HK_W)
        end

        if self.tyMenu.Combo.UseR:Value() and Ready(_R) and myHero.pos:DistanceTo(target.pos) <= 1150 then
...
function Leona:Harass()
    ...
    if IsValid(target) then
        ...
        if self.tyMenu.Harass.UseE:Value() and Ready(_E) and myHero.pos:DistanceTo(target.pos) <= 875 then
            local Pred = GetGamsteronPrediction(target, self.EData, myHero)
            if Pred.Hitchance >= _G.HITCHANCE_HIGH then
                NextTick = GetTickCount() + 250
                ORB:SetMovement(false)
        ...
        if self.tyMenu.Harass.UseW:Value() and Ready(_W) and myHero.pos:DistanceTo(target.pos) <= 325 then
            Control.CastSpell(HK_W)
        end
    ...
    for i = 1, #EnemyHeroes do
        ...
        if hero.activeSpell.spellWasCast then
            if ESpells[hero.activeSpell.name] ~= nil then
                if self.tyMenu.Auto.AotoEList[hero.charName]:Value() and hero.activeSpell.target == myHero.handle then
