...
function Leona:LoadMenu()
    LL = MenuElement({type = MENU, id = "ll", name = "Leona"})
...
function Leona:Draw()
    ...
    if LL.Combo.UseQ:Value() then
        Draw.Text("UseQ:true", myHero.pos2D.x, myHero.pos2D.y)
    else
        Draw.Text("UseQ:false", myHero.pos2D.x, myHero.pos2D.y)
    end
    if Ready(_Q) then
        Draw.Text("ReadyQ:true", myHero.pos2D.x, myHero.pos2D.y+20)
    else
        Draw.Text("ReadyQ:false", myHero.pos2D.x, myHero.pos2D.y+20)
    end

    if myHero.pos:DistanceTo(target.pos) <= 225 then
        Draw.Text("Distance:true", myHero.pos2D.x, myHero.pos2D.y+40)
    else
        Draw.Text("Distance:false", myHero.pos2D.x, myHero.pos2D.y+40)
    end

    if myHero.attackData.state == STATE_WINDDOWN then
        Draw.Text("state:true", myHero.pos2D.x, myHero.pos2D.y+60)
    else
        Draw.Text("state:false", myHero.pos2D.x, myHero.pos2D.y+60)
    end
--]]
...
function Leona:Combo()
    ...
    if IsValid(target) then
        ...
        and Ready(_Q) 
        and myHero.pos:DistanceTo(target.pos) <= 275 
        and myHero.attackData.state == STATE_WINDDOWN
        --and myHero.attackData.windDownTime >1
        and LocalGameTimer() > self.LastReset + 1
        then
        ...
        if LL.Combo.UseQ:Value() 
        and Ready(_Q) 
        and myHero.pathing.isDashing then
            Control.CastSpell(HK_Q)
        end
    end
...
function Leona:Harass()
    ...
    if IsValid(target) then
        ...
        if LL.Harass.UseQ:Value() and Ready(_Q) and myHero.pos:DistanceTo(target.pos) <= 275 and myHero.attackData.state == STATE_WINDDOWN then
            Control.CastSpell(HK_Q)
            ORB:__OnAutoAttackReset()
        end

        if LL.Harass.UseQ:Value() 
        and Ready(_Q) 
        and myHero.pathing.isDashing then
            Control.CastSpell(HK_Q)
        end
